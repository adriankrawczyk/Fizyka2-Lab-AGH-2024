...
# Wczytanie danych
data1 = pd.read_excel('INF3Z3ĆW134WYN.xlsx', sheet_name='MonokrystToPandas')
...
x_fit_mono = np.linspace(0, max(data1['U/n [V]']), 1000)

plt.figure(figsize=(10, 6))
plt.plot(data1['U/n [V]'], data1['j = I/S [mA/cm^2]'], 'o', color='blue')

plt.plot(x_fit_mono, exp_func(x_fit_mono, *popt1), 'r-')

plt.xlabel('U/n [V]')
plt.ylabel('j = I/S [mA/(cm)^2]')
plt.title('Wykres znormalizowanej charakterystyki: I/S = f(U/n) dla ogniwa monokrystalicznego')
plt.xlim(0.0, max((data1['U/n [V]'])))
plt.show()

# Wypisanie parametrów
print("\nParametry dopasowania (a, b, c):")
print(f"Monokrystaliczne: {popt1}")